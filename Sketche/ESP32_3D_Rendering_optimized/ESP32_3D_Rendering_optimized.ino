...
void connect(int i, int j, myvector *Points) {
    display.drawLine(Points[i].x, Points[i].y, Points[j].x, Points[j].y, SSD1306_WHITE);
}
...
void loop()
{
  ...
  for(int i=0;i<8;i++){
    updateRotation_X(angle, rotation);
    myvector rotated = matmul(rotation, Cube[i]);
    updateRotation_Y(angle, rotation);
    rotated = matmul(rotation, rotated);
    updateRotation_Z(angle, rotation);
    rotated = matmul(rotation, rotated);
